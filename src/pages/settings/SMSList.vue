<template>
  <div>
    <q-toolbar color="faded">
      ...
      <q-btn icon="edit" label="Edit Schedule Details" color="secondary" class="q-ml-sm q-ma-sm float-right"/>
      <q-btn icon="send" :label="'Send Messages ('+getMessagesLength+')'" color="primary" class="q-ml-sm q-ma-sm float-right" @click="sendMessages"/>
...
export default {
    ...
    mounted () {
      ...
      // console.log('contractsigned',this.contractsigned)
    },
    computed: {
      mapTemplates () {
        try {
      ...
      try{
        ...
        this.$bindAsArray('batchActivitys', this.$database.ref('event_schedules/'+ batch+'/activities'))

        let activity = this.$lodash.filter(this.batchActivitys, es=> {
          return es.id === id
        })
      ...
      getModels () {
        try {
          if (this.$route.params.type !== 'orientation' && this.$route.params.type !== 'payment') {  
            ...
            return modelList
          } else if (this.$route.params.type === 'payment') {
            console.log('payment')
    ...
    generateSMS (){
      ...
      for (var i = 0; i < models.length; i++) {
        ...
        var time = this.$moment(models[i].activityDate).format('LT')
    ...
    posting (n, a) {
      ...
      let total = this.messageList.length
      console.log('sms', SMS)
      let message = a
