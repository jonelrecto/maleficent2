...
<div>
      ...
      >
        ...
        <template slot="top-left" slot-scope="props">
          <div class=""> 
          <h4 class="q-mr-lg">Upcoming Schedule</h4><br>
        ...
        unchecked-icon="visibility_off"/>        
          ...
          color="secondary"
          v-model="searchName"
          stack-label="Search By Model's Name"
          class="col-5"
          />
        </template>
...
<!--     fullday activityList -->
    <div class="col-3 q-ma-md" v-if="fullDayShow">
      ...
      >
        ...
        <template slot="top-left" slot-scope="props">
          <div class=""> 
          <h4 class="q-mr-lg">Upcoming Schedule<span class="text-secondary text-weight-bold"> Day View</span></h4><br>
    ...
    computed: {
      ...
      mapEventActivities () {
        ...
        let filterPayment = this.$lodash.map(this.getActivities, a => {
            if (a.type != 'payment' && a.type != 'payments') {
      ...
      getMapActivities () {
        ...
        return {
          ...
          models: this.returnModels(es['.key'],this.upcomingActivity(es.activities).id,this.upcomingActivity(es.activities).type)
        }
      })
      console.log('getMapActivities',reconst)
      return reconst
      },
      getAllAttendance () {
        try {
              ...
              for(var y=0;y<openbatches.length;y++) {
                if(activities[x].batchKey === openbatches[y]['.key']){
                  let push = {
                      ...
                      models: this.returnModels(openbatches[y]['.key'],activities[x].id,activities[x].type)
                  } 
                  filtered.push(push)
                } else {
      ...
      getFilteredActivities () {
        if(this.seeAllFullDay === true){
          ...
          // console.log('getFilteredActivities', filtered)
          return filtered
      ...
      ByBatchTableData () {
        try {
            ...
            if(this.seeAll === true) {
              ...
              // console.log('ByBatchTableData',upcoming)
              return upcoming
            }
          } catch (err) {
            console.log('err',err)
      ...
      filteredByBatchTableData () {
        //nameSearchMethod
        try {
          if(this.searchName !== ''){
            let schedules = this.ByBatchTableData
            let searched = this.searchName

            let filteredbatchwithmodels = this.$lodash.filter(schedules, e=>{
              return e.models
            })

            console.log('filteredbatchwithmodels',filteredbatchwithmodels)
            let checked = []
            for(var i = 0; i < filteredbatchwithmodels.length; i++) {
              let batch = filteredbatchwithmodels[i].models
              console.log('batch',batch)
              let modelschecker = this.$lodash.filter(batch, es=>{
                var reg = new RegExp(searched, "i")
                var string = es.firstName +' '+ es.lastName
                if(string.match(reg)){
                  checked.push(filteredbatchwithmodels[i])
                } else {
                  return 0
                }
              })
            }
            console.log('checkedmodels',checked)
            return checked
          } else {
            return this.ByBatchTableData
          }
