	...
	<q-page>
	  ...
	  >
	    ...
	    <template slot="top-right" slot-scope="props">
	    	<div class="q-px-md">
	    		<p>Not Paid Models: {{mappedModels.length}}</p>
	    	</div>
	...
	computed: {
		mappedModels () {
			...
			let filterModels = this.$lodash.filter(mapmodels, mm => {
				return mm.lastPaidDate === 'Not Paid'
			})
		...
		print () {
			...
			var dd = {
				...
				content: [
					...
					{text: 'Payment Records: Not Paid', alignment: 'left', margin: [5, 5, 5, 5]},
					{
						style: 'tableExample',
						table: {
							headerRows: 1,
					...
					{text: `Not Paid: ${this.mappedModels.length}`, alignment: 'left', margin: [1, 1, 1, 1]}
				],
				styles: {
					header: {
						fontSize: 18,
			...
			console.log('dd', dd)
			pdfMake.createPdf(dd).download('PaymentReports_NotPaid');
		},
		test () {
