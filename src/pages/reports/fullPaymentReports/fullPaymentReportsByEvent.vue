<template>
	<q-page>
	    ...
	    :filter="filter"
	    row-key="name"
	    no-data-label="No Full Paid Models"
		...
		print () {
			...
			var dd = {
				...
				content: [
					...
					{text: `Payment Records: Full Payment - ${this.eventName} - ${this.selectedSeason}`, alignment: 'left', margin: [5, 5, 5, 5]},
					{
						style: 'tableExample',
						table: {
							headerRows: 1,
			...
			console.log('dd', dd)
			pdfMake.createPdf(dd).download(`PaymentReports_FullPaid_${this.eventName}_${this.selectedSeason}`);
		},
		test () {
