	...
	<q-page>
	  ...
	  >
	    ...
	    <template slot="top-right" slot-scope="props">
	    	<div class="q-px-md">
	    		<p>Models: {{mappedModels.length}}</p>
				  <p>Full Paid: {{getFullPaidModels.length}}</p>
				  <p>Installment: {{getInstallmentModels.length}}</p>
				  <p>Not Paid: {{getNotPaidModels.length}}</p>
	...
	computed: {
		mappedModels () {
			let filterModels = this.$lodash.filter(this.contractSigned, cs => {
				return cs.eventKey === this.eventKey
			})
      ...
      return isNan.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
		...
		print () {
			...
			for (var i = 0; i < models.length; i++) {
				...
				let toPush = [models[i].fullName, models[i].eventName, models[i].seasonAndBatch, models[i].seasonRate, term, models[i].lastPaidDate, models[i].totalPaid]
				toBody.push(toPush)
			}
			let vm = this
			...
			var dd = {
				...
				content: [
					...
					{text: `Payment Records: ${eventName}`, alignment: 'left', margin: [5, 5, 5, 5]},
					{
						style: 'tableExample',
						table: {
							headerRows: 1,
					...
					{text: `Models: ${this.mappedModels.length}`, alignment: 'left', margin: [1, 1, 1, 1]},
						{text: `Full Paid: ${this.getFullPaidModels.length}`, alignment: 'left', margin: [1, 1, 1, 1]},
					{text: `Installment: ${this.getInstallmentModels.length}`, alignment: 'left', margin: [1, 1, 1, 1]},
					{text: `Not Paid: ${this.getNotPaidModels.length}`, alignment: 'left', margin: [1, 1, 1, 1]},
			...
			console.log('dd', dd)
			pdfMake.createPdf(dd).download(`PaymentReports_AllPayments_${eventName}`);
		},
		test () {
