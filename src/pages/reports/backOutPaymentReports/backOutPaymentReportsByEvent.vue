	...
	<q-page>
	  ...
	  >
	    ...
	    <template slot="top-right" slot-scope="props">
	    	<div class="q-px-md">
	    		<p>Backout Models: {{mappedModels.length}}</p>
...
export default {
	...
	computed: {
		mappedModels () {
			let filterModels = this.$lodash.filter(this.backOutModels, cs => {
		...
		print () {
			...
			var dd = {
				...
				content: [
					...
					{text: `Payment Records: Backout - ${eventName}`, alignment: 'left', margin: [5, 5, 5, 5]},
					{
						style: 'tableExample',
						table: {
							headerRows: 1,
					...
					{text: `Backout Models: ${this.mappedModels.length}`, alignment: 'left', margin: [1, 1, 1, 1]},
			...
			console.log('dd', dd)
			pdfMake.createPdf(dd).download(`PaymentReports_Backout_${eventName}`);
		},
		test () {
