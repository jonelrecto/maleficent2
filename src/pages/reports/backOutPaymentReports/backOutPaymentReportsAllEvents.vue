<template>
	<q-page>
		<q-toggle v-model="isAdvanceDate" label="Advance Date to 2019-02-01" />
	  ...
	  >
	    ...
	    <template slot="top-right" slot-scope="props">
	    	<div class="q-px-md">
	    		<p>Back Out Models: {{mappedModels.length}}</p>
...
export default {
	...
	computed: {
		mappedModels () {
			let mapmodels = this.$lodash.map(this.backOutModels, cs => {
		...
		finishedSeasons () {
			...
			let finishedSeasons = this.$lodash.filter(seasons, s => {
				let dif = this.$mathMixin.dayDiff(new Date(this.advanceDate), new Date(s.seasonEnd))
				if (dif < 0) {
					return s
				}
			})
			// console.log('finishedSeasons', finishedSeasons)
			return finishedSeasons
		},
		backOutModels () {
			let backOutModels = []
			this.$lodash.forEach(this.contractSigned, cs => {
				this.$lodash.forEach(this.finishedSeasons, fs => {
					if (cs.seasonName === fs.seasonName && cs.balance !== 0) {
						backOutModels.push(cs)
					}
				})
			})
			// console.log('backout models', backOutModels)
			return backOutModels
		}
	},
	data () {
		return {
        ...
        { name: 'paymentTerm', align: 'left', label: 'Payment Term', field: 'paymentTerms', sortable: true},
        { name: 'lastPaidDate', align: 'left', label: 'Last Paid Date', sortable: true, field: 'lastPaidDate'},
        { name: 'balance', align: 'left', label: 'Remaining Balance', sortable: true, field: 'balance'},
      ...
      return isNan.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
		...
		print () {
			...
			for (var i = 0; i < models.length; i++) {
				...
				let toPush = [models[i].fullName, models[i].eventName, models[i].seasonAndBatch, models[i].seasonRate, term, models[i].lastPaidDate, models[i].balance, models[i].totalPaid]
				toBody.push(toPush)
			}
			let vm = this
			...
			var dd = {
				...
				content: [
					...
					{text: 'Payment Records: Back Out - All Events', alignment: 'left', margin: [5, 5, 5, 5]},
					{
						style: 'tableExample',
						table: {
							headerRows: 1,
					...
					{text: `Back Out: ${this.mappedModels.length}`, alignment: 'left', margin: [1, 1, 1, 1]},
			...
			console.log('dd', dd)
			pdfMake.createPdf(dd).download('PaymentReports_Backout_AllEvents');
		},
		test () {
			console.log('backOutModels', this.backOutModels)
