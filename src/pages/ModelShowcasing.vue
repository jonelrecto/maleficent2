...
created () {
    this.codeShowcase = this.$route.params.id
    // console.log('cs', this.contractsigned)
  },
  computed: {
    ...
    getModelFromShowcasing () {
      console.log('showcasing',this.showcasing)
      let models = []
      let filter = this.$lodash.filter(this.showcasing, e=> {
        if(e.id === this.codeShowcase) {
          models.push(e.models)
        }
        return e
      })
      console.log('this.models',models[0])
      return models[0]
    },
    getToday () {
    ...
    getInhouseSorted () {
      try{
        let inhouse = this.$lodash.map(this.getModelFromShowcasing, as=> {
    ...
    getAttendancePresents () {
     ...
     getSelectedRowCount () {
        ...
        console.log('gallery',this.selectedGallery)
        return this.selected.length
      },
      get1st () {
      try {
        let modelList = this.$lodash.filter(this.getModelFromShowcasing, c => {
          c.name = c.firstName
          return c
        })
        console.log(modelList)
    ...
    getPhotoGallerySelected () {
      try {
        ...
        let gallery = this.$lodash.filter(this.getModelFromShowcasing, e=>{
          return e.modelKey === model.modelKey
        })
      ...
      try {
        this.loading = true
          if(this.selected.length === 0) {
            ...
            let key = this.$lodash.find(this.showcasing,e=>{
              return e['.key']
            })
            ...
            this.$database.ref('showcasing/'+skey+'/selected/').set(porfolio)
          .then(()=>{
               this.$q.notify({
