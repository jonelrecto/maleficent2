...
export const generateSeason = ({state}, {batches, season, event}) => {
	return new Promise((resolve, reject) => {
		let db = Vue.prototype.$database
		console.log(batches, 'batches')
		console.log(season, 'season')
		console.log(event, 'event')
		season.batches = batches
		let key = event['.key']
		delete event['.key']
		...
		function saveBatches () {
			// setTimeout(() => {
				console.log(batches)
				if (i < batches.length) {
					console.log(i, batches[i] , 'delay')
					// i++
					// saveBatches()
					db.ref('event_schedules').push(batches[i])
						.then(() => {
							i++
							saveBatches()
						})
		...
		if (event.seasons) {
			event.seasons.push(season)
			
			console.log('with season')
			console.log(event, 'to upadte')
			db.ref(`events/${key}`).update(event)
				.then(() => {
					saveBatches()
...
export const generateSeasonBatches = ({state}, batches) => {
