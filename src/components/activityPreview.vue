	...
	<div class="row flex justify-center">
	  ...
	  <div class="col">
			...
			<div class="a">
 				...
 				<q-toggle v-if="isNewBatch" v-model="viewBatchesChecked" @input="onChangeViewBatches" label="View Batches" />
			</div>
...
export default {
	...
	created () {
		...
		console.log(this.localPlottedActivities, 'localPlottedActivities')
	},
	watch: {
		allScheduleChecked () {
	...
	methods: {
		...
		saveNewSeasonBatchActivity: async function () {
			...
			let result = await promise
			console.log('saveNewSeasonBatchActivity ,result ', result)	
			return result
		},
		...
		saveNewBatch: async function () {
			let promise =  new Promise(resolve => {
				...
				let activities = this.$store.getters['startUp/getPlainValue'](this.localPlottedActivities)

				newBatch.activities =  activities
				delete newBatch['.key']
		...
		saveNewSeason () {
			// console.log('event schedules save new season', this.eventSchedules)
  		this.removeOriginalActivityField()
			...
			// 	activities[i].startDate = activities[i].startDate.toString()
			// 	activities[i].endDate = activities[i].endDate.toString()
			// 	activities[i].time = activities[i].time.toString()
			// }
			// let act2 = this.convertToStringDates(activities)
		...
		test (act) {
			console.log('localPlottedActivities', this.localPlottedActivities)
